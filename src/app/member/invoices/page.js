...
export default function InvoicesLog() {
  ...
  const handlePayInvoice = (id) => {
    ...
    // If details drawer is open, update its state
    ...
    <div style={{ display: 'grid', gap: '32px', position: 'relative' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '20px' }}>
        <div style={{ background: 'var(--mars-slate)', padding: '20px', borderRadius: '6px', border: '1px solid var(--line-dark)' }}>
          <div style={{ fontSize: '13px', color: 'var(--text-muted-dark)' }}>{language === 'ar' ? 'المبلغ المستحق' : 'Outstanding Balance'}</div>
          <div style={{ fontSize: '22px', fontWeight: 700, color: '#FFFFFF', marginTop: '6px' }}>
            {invoices.filter(i => i.status === 'Unpaid').reduce((a, b) => a + b.amount, 0)} SAR
          </div>
        </div>
        <div style={{ background: 'var(--mars-slate)', padding: '20px', borderRadius: '6px', border: '1px solid var(--line-dark)' }}>
          <div style={{ fontSize: '13px', color: 'var(--text-muted-dark)' }}>{language === 'ar' ? 'المدفوعات هذا العام' : 'Paid This Year'}</div>
          <div style={{ fontSize: '22px', fontWeight: 700, color: '#FFFFFF', marginTop: '6px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', textAlign: 'start' }}>
          ...
          <tbody>
            {filteredInvoices.map((inv) => {
              ...
              return (
                <tr key={inv.id} style={{ borderBottom: '1px solid rgba(245, 245, 245, 0.04)' }}>
                  <td style={{ padding: '16px 24px', fontWeight: 600 }}>{inv.invoiceNumber}</td>
                  <td style={{ padding: '16px 24px' }}>{inv.type}</td>
                  <td style={{ padding: '16px 24px' }}>{inv.dueDate}</td>
                  <td style={{ padding: '16px 24px', fontWeight: 600 }}>{inv.amount} SAR</td>
      ...
      {selectedInvoice && (
        <div style={{
          ...
          {/* Drawer Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(245, 245, 245, 0.08)', paddingBottom: '20px' }}>
            <h3 style={{ margin: 0, fontSize: '20px', color: '#FFFFFF' }}>{selectedInvoice.invoiceNumber}</h3>
            <button
          ...
          {/* Drawer Body details */}
          <div style={{ flex: 1, overflowY: 'auto', marginTop: '24px', display: 'grid', gap: '20px', fontSize: '14px' }}>
            
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div>
                <div style={{ color: 'var(--text-muted-dark)' }}>{language === 'ar' ? 'تاريخ الفاتورة' : 'Invoice Date'}</div>
                <div style={{ fontWeight: 600, color: '#FFFFFF', marginTop: '4px' }}>{selectedInvoice.date}</div>
              </div>
              <div>
                <div style={{ color: 'var(--text-muted-dark)' }}>{language === 'ar' ? 'تاريخ الاستحقاق' : 'Due Date'}</div>
                <div style={{ fontWeight: 600, color: '#FFFFFF', marginTop: '4px' }}>{selectedInvoice.dueDate}</div>
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div>
                <div style={{ color: 'var(--text-muted-dark)' }}>{language === 'ar' ? 'الحالة' : 'Status'}</div>
                <div style={{ fontWeight: 600, color: selectedInvoice.status === 'Paid' ? '#4CAF50' : '#FF4A4A', marginTop: '4px' }}>
                  {selectedInvoice.status}
                </div>
              </div>
              <div>
                <div style={{ color: 'var(--text-muted-dark)' }}>{language === 'ar' ? 'وسيلة الدفع' : 'Payment Method'}</div>
                <div style={{ fontWeight: 600, color: '#FFFFFF', marginTop: '4px' }}>{selectedInvoice.paymentMethod}</div>
              </div>
            </div>

            {/* Line items list */}
            <div style={{ borderTop: '1px solid rgba(245, 245, 245, 0.08)', paddingTop: '20px' }}>
              <div style={{ fontWeight: 600, color: '#FFFFFF', marginBottom: '12px' }}>{language === 'ar' ? 'تفاصيل الرسوم' : 'Line Items'}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ color: 'var(--text-muted-dark)' }}>{selectedInvoice.description}</span>
                <span style={{ fontWeight: 600 }}>{selectedInvoice.subtotal} SAR</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ color: 'var(--text-muted-dark)' }}>VAT (15%)</span>
                <span style={{ fontWeight: 600 }}>{selectedInvoice.vat} SAR</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(245, 245, 245, 0.05)', paddingTop: '10px', fontSize: '15px', fontWeight: 700, color: '#FFFFFF' }}>
